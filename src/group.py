...
'''
# Для визуализации кластеров, связанных с квадруплексами, были взяты представители рода Eimeria
feature_tables = {}
for i in ["eimeria_acervulina", "eimeria_brunetti", "eimeria_maxima", "eimeria_mitis", "eimeria_necatrix", "eimeria_praecox"]:
    path_to_ft = "/home/leonid/PycharmProjects/python/minor_2022/project/supergroup/" + i + "_feature_table.txt"
    feature_tables[i] = pd.read_csv(path_to_ft, sep='\t', dtype=str)
...
view_clusters = pd.read_csv("clusters_for_heatmap.tsv", sep='\t')
for index, row in view_clusters.head(4).iterrows():
    name_for_image = "cluster" + str(index + 1) + ".png"
    data_for_features = {"q_dna_start": [], "q_dna_end": [], "length": [],
                         "gene_start": [], "gene_end": [], "strand": [], "name": []}
    for i in ["eimeria_acervulina", "eimeria_brunetti", "eimeria_maxima", "eimeria_mitis", "eimeria_necatrix", "eimeria_praecox"]:
        name_for_label = i + "_protein.faa"
        protein_ID = row[name_for_label].split(',')[0]
        if protein_ID == '*':
            continue
        else:
            data_for_features["name"].append(row[name_for_label] + " " + i)
            if protein_ID in q_data[i]["ID"].tolist():
                q_data_index = q_data[i].index[q_data[i]["ID"] == protein_ID].tolist()[0]
            ...
            feature_index = feature_tables[i].index[feature_tables[i]["product_accession"] == protein_ID]
            feature_index = int(feature_index[0])
    ...
    for i in range(5):
        if data_for_features["strand"][i] == -1:
            data_for_features["strand"][i] = +1
            new_q_start = data_for_features["length"][i] - data_for_features["q_dna_end"][i]
            new_q_end = data_for_features["length"][i] - data_for_features["q_dna_start"][i]
            data_for_features["q_dna_start"][i] = new_q_start
            data_for_features["q_dna_end"][i] = new_q_end
    ...
    feature1 = [GraphicFeature(start=data_for_features["q_dna_start"][0], end=data_for_features["q_dna_end"][0],
                ...
                GraphicFeature(start=data_for_features["gene_start"][0], end=data_for_features["gene_end"][0],
                               strand=data_for_features["strand"][0], color="#ffcccc", label=data_for_features["name"][0])]
    ...
    feature2 = [GraphicFeature(start=data_for_features["q_dna_start"][1], end=data_for_features["q_dna_end"][1],
                ...
                GraphicFeature(start=data_for_features["gene_start"][1], end=data_for_features["gene_end"][1],
                               ...
                               label=data_for_features["name"][1])]
    ...
    feature3 = [GraphicFeature(start=data_for_features["q_dna_start"][2], end=data_for_features["q_dna_end"][2],
                ...
                GraphicFeature(start=data_for_features["gene_start"][2], end=data_for_features["gene_end"][2],
                               ...
                               label=data_for_features["name"][2])]
    ...
    feature4 = [GraphicFeature(start=data_for_features["q_dna_start"][3], end=data_for_features["q_dna_end"][3],
                ...
                GraphicFeature(start=data_for_features["gene_start"][3], end=data_for_features["gene_end"][3],
                               ...
                               label=data_for_features["name"][3])]
    ...
    feature5 = [GraphicFeature(start=data_for_features["q_dna_start"][4], end=data_for_features["q_dna_end"][4],
                ...
                GraphicFeature(start=data_for_features["gene_start"][4], end=data_for_features["gene_end"][4],
                               ...
                               label=data_for_features["name"][4])]
